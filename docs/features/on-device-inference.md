...
The allowlist (in `RemoteDataRepository.LOCAL_TOOL_ALLOWLIST`) currently exposes: `get_local_time`, `get_location_from_ip`, `web_search`, `open_url`, `memory_store`, `memory_forget`, `memory_reinforce`, `execute_shell_command` (when the user has enabled the shell tool in Settings), and the email **send** tools `compose_email`, `reply_email`, and `send_outreach_email`. The send tools take only a few string params, so a small Gemma can drive them — there's no reason to force a cloud model for everyday send/reply/outreach; incoming mail still reaches the model via the heartbeat's email context. `send_outreach_email` is the guardrailed path for cold/bulk sends (dedup against already-contacted addresses + a hard per-day cap enforced in code). Email **read/search** tools (`check_email` / `read_email` / `search_email`), task scheduling (`schedule_task` / `list_tasks` / `cancel_task`), MCP server tools, structured `memory_learn`, heartbeat-config tools, and `promote_learning` are excluded — they require a remote model.
